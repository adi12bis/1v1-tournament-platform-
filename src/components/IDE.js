    ...
    const handleMessage = async (event) => {
      ...
      if (event.origin === "https://www.jdoodle.com" && event.data.script) {
        ...
        try {
          ...
          const GEMINI_API_KEY = 'AIzaSyA42FFNZtpiukLr0PFhUsfBuTibYMdAqOY';
          const ai = new GoogleGenAI({ apiKey : GEMINI_API_KEY });
          const codeReviewResponse = await ai.models.generateContent({
            model: 'gemini-2.0-flash',
            contents: `Analyze this code and write a single paragraph long review. Only give the review in your response. \n${script}`
          });
          localStorage.setItem('userCodeReview', codeReviewResponse.text);
